      ...
      if ('auth_token' in postData && postData.auth_token == authToken) {
        if (!'base_commands' in postData) {
          ...
          if (!'label_commands' in postData) {
            ...
            var postDataBaseCommands = querystring.parse(
              postData.base_commands
            );
            ...
            Object.keys(postDataBaseCommands).forEach(function (key) {
              base_commands.push(
                Buffer.concat([
                  new Buffer([0x02], 'hex'),
                  new Buffer(postDataBaseCommands[key]),
                ])
              );
            });
            ...
            var postDataLabelCommands = querystring.parse(
              postData.label_commands
            );
