export function moneyEvaluation(printer: any) {
  ...
  printer?.setTextDoubleHeight();
  printer?.setTextDoubleWidth();
  printer?.println('RECIBO/DINHEIRO');
  printer.setTextNormal(); 
  printer?.drawLine(); 
  printer?.newLine();
  printer?.alignLeft(); 
  printer?.println('10/03/2023 09:59:20')             //data avaliação
  printer?.println('Eu "Nome cliente", CPF n° "111.111.110-30", declaro que por ter vendido produtos seminovos para a Cresci e Perdi, recebi a quantia de "R$ 10,00 (DEZ REAIS)", em Dinheiro. Declaro que os itens vendidos à CRESCI E PERDI são de minha propriedade e estou ciente que serão por ela comercializados a outrem, sem que eu tenha participação no negócio. Declaro por fim, que tive tempo hábil para avaliar o negócio que fiz, tomei minha decisão de forma absolutamente consciente, não podendo dela legalmente me arrepender.');
  ...
  // printer?.alignCenter();
  // printer?.newLine();
  // printer?.println('UTILIZANDO O GIRACRÉDITO VOCÊ OBTÉM PREÇOS');    VIA CLIENTE
